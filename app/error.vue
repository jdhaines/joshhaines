...
<template>
  <UApp>
    ...
    <UContainer
      class="flex min-h-[60vh] flex-col items-center justify-center py-24 text-center"
    >
      <div class="mb-6 flex items-center gap-3">
        <span class="h-px w-8 shrink-0 bg-primary" aria-hidden="true" />
        <span
          class="font-mono text-sm font-bold tracking-[0.2em] text-highlighted uppercase"
        >
          {{ statusCode }}
        </span>
        <span class="h-px w-8 shrink-0 bg-primary" aria-hidden="true" />
      ...
      <p class="mb-8 max-w-prose text-lg text-muted">
        We've checked everywhere and there's no sign of it. Josh definitely didn't break
        a link, so we've sent the intern to go dig through the server logs and figure
        out what happened. In the meantime, here are a few places that are still exactly
