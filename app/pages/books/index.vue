...
const { ranked, mentions } = computed(() =>
  buildBookShelf(shelf.value, posts.value ?? [])
).value
...
const filteredMentions = computed(() =>
  searchBooks(mentions, query.value).sort((a, b) =>
...
const description =
  "Every book I\u2019ve read and reviewed, ranked from most useful to least useful."
...
<template>
  <UContainer class="py-12">
    <header class="mb-10 max-w-2xl">
      <h1 class="mb-3 font-serif text-4xl font-bold">Book Shelf</h1>
      <p class="text-lg text-muted">
        In order of <strong>most useful</strong> to <strong>least useful</strong>.
        Switch the sort below to browse by title, author, or review date instead -- the
    ...
    <div
      class="mb-8 flex flex-col gap-4 border-y border-default py-4 sm:flex-row sm:items-center sm:justify-between"
    >
      <UInput
        v-model="query"
    ...
    <p v-if="!filteredRanked.length" class="py-12 text-center text-muted">
      No books match "{{ query }}".
    </p>

    <ol v-else-if="view === 'list'" class="divide-y divide-default">
      <li v-for="book in filteredRanked" :key="book.post.path">
        <NuxtLink :to="book.post.path" class="group flex items-center gap-4 py-4">
    ...
    <ul v-else class="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-3 lg:grid-cols-4">
      <li v-for="book in filteredRanked" :key="book.post.path">
        <NuxtLink :to="book.post.path" class="group block">
          <div class="relative">
            ...
            <span
              class="absolute top-1 left-1 flex size-8 items-center justify-center rounded-full bg-secondary font-mono text-sm font-medium text-inverted"
            >
              {{ book.rank }}
            </span>
          </div>
    ...
    <section
      ...
      class="mt-16"
    >
      <HomeSectionEyebrow id="honorable-mentions-heading" label="Honorable Mentions" />
      <p class="mb-6 max-w-2xl text-muted">
        Outside the main ranking -- outstanding books on unrelated topics, listed
        alphabetically.
      </p>
