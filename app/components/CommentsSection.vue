...
const { data: comments, refresh: refreshComments } = await useAsyncData(
  `comments-${props.path}`,
  () =>
    $fetch<{ comments: CommentItem[] }>("/api/comments", {
...
async function submitComment() {
  ...
  if (
    ...
    !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(authorEmail.value.trim())
  ) {
...
<template>
  ...
  >
    <h2 id="comments-heading" class="font-serif text-2xl font-semibold">Comments</h2>
      ...
      >
        <div class="flex items-baseline justify-between gap-4">
          <span class="font-medium">
            ...
            <span v-if="comment.author_email" class="font-normal text-muted">
              ({{ comment.author_email }})</span
            >
          </span>
          <time class="text-sm text-muted" :datetime="comment.created_at">{{
